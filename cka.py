#!/usr/bin/env python3
# -*- coding: utf-8 -*-


import grpc
import weladee_pb2
import weladee_pb2_grpc
import os
import logging
from time import sleep
import urllib
import cStringIO
import base64
from PIL import Image
import requests
from datetime import datetime,date, timedelta


def main():
    print("Test grpc weladee")


    # Weladee grpc server address is hrpc.weladee.com:22443
    address = "grpc.weladee.com:22443"

    # Define a secure channel with embedded public certificate

    creds = grpc.ssl_channel_credentials(certificate)
    channel = grpc.secure_channel(address, creds)


    # We create one channel to the server then we can use for several functions
    stub = weladee_pb2_grpc.MobileStub(channel)
    # Here you define the token. It's always sent as metadata to all requests

    # Connect from mobile device

    #token = [("token", "88917ec1-a589-4891-bb64-b10ca0afd666")]


    myrequest = weladee_pb2.EmployeeRequest()




    # Connect from Odoo
    # Place here the token specific to each company. It's called api_key in table company

    authorization = [("authorization", "bc7f3c00-bfa4-4ac2-810b-a11dca5ec48e")]

    stub = weladee_pb2_grpc.OdooStub(channel)

    # List all departments
    print("Departments")
    if False:
        for dept in stub.GetDepartments(myrequest, metadata=authorization):
            print(dept)

    # List of employees
    print("Employees")
    if False :
        for emp in stub.GetEmployees(weladee_pb2.Empty(), metadata=authorization):
            print(emp)

    # List of GetPositions
    print("Positions")
    if False :
        for pos in stub.GetPositions(weladee_pb2.Empty(), metadata=authorization):
            print(pos)

    # List of CompanyHolidays
    print("CompanyHolidays")
    if False :
        for ch in stub.GetCompanyHolidays(weladee_pb2.Empty(), metadata=authorization):
            print(ch)

    # List of CompanyHolidays
    print("Holidays")
    """h = "2017-07-25 00:00:00"
    dh = datetime.strptime( h, "%Y-%m-%d %H:%M:%S" )
    print(dh)
    print( dh.strftime("%Y%m%d")  )
    print( int( dh.strftime("%Y%m%d") )  )

    d1 = date(2017, 7, 25)  # start date
    d2 = date(2017, 7, 31)  # end date

    delta = d2 - d1         # timedelta

    for i in range(delta.days + 1):
        print(d1 + timedelta(days=i))"""

    print("akaa")
    emp = weladee_pb2.OdooRequest()
    emp.odoo_id = 1
    print( emp )
    try:
        for ch in stub.GetHolidays(emp, metadata=authorization):
            print(ch)
    except Exception as e:
        print("Add holiday failed", e)



    # List of attendance to sync
    #print("Attendance to sync")
    #i=0
    #for att in stub.GetNewAttendance(weladee_pb2.Empty(), metadata=authorization):
        #i+=1
        #logging.log(i,att)


    print("Holiday")
    sHoliday = {}
    print("Holiday company")
    #for hol in stub.GetCompanyHolidays(weladee_pb2.Empty(), metadata=authorization):
        #print("Holiday company")
        #print(hol)

    print("Test update employee")
    if False :
        newEmployee = weladee_pb2.EmployeeOdoo()
        newEmployee.odoo.odoo_id = 388
        newEmployee.odoo.odoo_created_on = 1500537287
        newEmployee.odoo.odoo_synced_on = 1500537287

        newEmployee.employee.ID = 2056
        newEmployee.employee.user_name = "dabcnomvllurfrscksyiqr"
        newEmployee.employee.last_name_english = "Cyre"
        newEmployee.employee.first_name_english = "Popi"
        newEmployee.employee.last_name_thai = "cyre"
        newEmployee.employee.active = True
        newEmployee.employee.created_on = -62135596800
        newEmployee.employee.updated_on = -62135596800
        newEmployee.employee.lg = "en"
        newEmployee.employee.application_level = 0
        newEmployee.employee.positionid = 452343311

        try:
            result = stub.UpdateEmployee(newEmployee, metadata=authorization)
            print ("Update odoo employee id : %s" % result.id)
        except Exception as e:
            print("Update odoo employee id",e)

    if False :
        print("Test update employee")

        newPosition = weladee_pb2.PositionOdoo()
        newPosition.odoo.odoo_id = odooPositions[pName]
        newPosition.odoo.odoo_created_on = int(time.time())
        newPosition.odoo.odoo_synced_on = int(time.time())
        newPosition.position.name_english = pName
        newPosition.position.active = True
        print(newPosition)
        try:
            result = stub.AddPosition(newPosition, metadata=authorization)
            print ("Add position : %s" % pName)
        except Exception as e:
            print("Add position failed",e)


    if False :
        newEmployee = weladee_pb2.EmployeeOdoo()
        newEmployee.odoo.odoo_id = 3888
        newEmployee.odoo.odoo_created_on = 1500537288
        newEmployee.odoo.odoo_synced_on = 1500537288

        newEmployee.employee.ID = 2056
        newEmployee.employee.user_name = "dabcnomvllurfrscksyiqr"
        newEmployee.employee.last_name_english = "Cyre"
        newEmployee.employee.first_name_english = "Popi"
        newEmployee.employee.last_name_thai = "cyre"
        newEmployee.employee.active = True
        newEmployee.employee.lg = "en"
        newEmployee.employee.application_level = 1
        newEmployee.employee.positionid = 452343311

        print( "CKAtest" )
        print( newEmployee )

        try:
            result = stub.UpdateEmployee(newEmployee, metadata=authorization)
            print("Update odoo employee id : %s" % result.id)
        except Exception as e:
            print("Update odoo employee id is failed", e)

# Certificate to be downloaded from https://git.frontware.co.th/raw/Weladee/proto.git/master/certificates!grpc.weladee.com.chain.crt

certificate = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

Ob8VZRzI9neWagqNdwvYkQsEjgfbKbYK7p2CNTUQ
-----END CERTIFICATE-----
"""


if __name__ == "__main__":
    import sys

    if '--optimize' in sys.argv:  # if --optimize then restart application with -O parameter.
        sys.argv.remove('--optimize')
        os.execl(sys.executable, sys.executable, '-O', *sys.argv)
    else:
        main()
